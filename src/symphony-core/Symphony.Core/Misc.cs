...
namespace Symphony.Core
{
    ...
    // TODO: Use .NET library implementation when they create it.
    /// <summary>
    /// A simple thread-safe double-ended queue.
    /// </summary>
    /// <remarks>This implementation currently uses thread locking for simplicity.</remarks>
    /// <typeparam name="T">The type of elements contained in the deque.</typeparam>
    public class ConcurrentDeque<T> : IProducerConsumerCollection<T>, IEnumerable<T>, ICollection, IEnumerable
    {
        ...
        {
            _items = new LinkedList<T>(collection);
        }
        ...
        {
            lock (_syncLock) _items.AddFirst(item);
        }

        public void EnqueueLast(T item)
        {
            lock (_syncLock) _items.AddLast(item);
        }
        ...
        {
            lock (_syncLock) ((ICollection)_items).CopyTo(array, index);
        }
        ...
        {
            lock (_syncLock) _items.CopyTo(array, index);
        }
        ...
        {
            return TryDequeueFirst(out item);
        }
